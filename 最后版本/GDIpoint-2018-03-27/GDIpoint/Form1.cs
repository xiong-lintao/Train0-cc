...
namespace GDIpoint
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            IList<XY> ggList = new List<XY>();//下行
            ...
            proportionY = 1000 / spaceExtra;//比例值
            ...
            g.DrawString("上行", font, Brushes.Red, 30, 25);
            ...
            Array.Reverse(space);
            Array.Reverse(runtimes);
            float XX = pointDowns[pointDowns.Length-1].X;
            for (int i = 1; i < sLength - 2; i++)
            {
                if(XX < x-40 ){
                    if (flag)
            ...
            PointF[] points1 = new PointF[ggList.Count + 1];
            points1[0] = pointDowns[pointDowns.Length-1];


            for (int i = 0; i < ggList.Count; i++)
            {
                points1[i + 1] = new PointF(Convert.ToSingle(ggList[i].X), Convert.ToSingle(ggList[i].Y));
            }

            g.DrawLines(mypenRed, points1);
